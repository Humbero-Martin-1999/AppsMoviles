            ...
            items(accountsdb) { accountdb ->
                MaterialTheme(
                    colorScheme = MaterialTheme.colorScheme.copy(
                        // Suponiendo que FavoriteAccountCard usa surfaceVariant para el fondo
                        surfaceVariant = Color(0xFF73C2D7),  // azul claro
                        onSurfaceVariant = Color.Black      // texto u contenido en negro (o el que prefieras)
                    )
                ) {
                    FavoriteAccountCard(
                        accountdb.id ?: 0,
